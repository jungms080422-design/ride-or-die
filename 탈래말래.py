...
if not st.session_state.logged_in:
    ...
    alert_time_input = st.sidebar.time_input("알림 시간:", default_time)
    ...
    selected_time = st.sidebar.time_input("예약 시간:", datetime.datetime.now().time(), key="reserve_time")
    ...
    steps_to_add = st.sidebar.number_input("추가할 걸음 수 입력:", min_value=0, max_value=10000, value=0, step=100, key="step_input_main")
    if steps_to_add > 0:
        simulate_steps_logic(steps_to_add)
        st.session_state.step_input_main = 0 
 ...
 # B1, 1F, 2F
    ...
    for i in range(3): 
        ...
        with cols_top[i]:
            ...
            if reservation_list: # 리스트에 1명이라도 예약이 있으면
                # (수정) 예약 인원 수 계산
                count = len(reservation_list)
                
                # (수정) 가장 빠른 예약 시간 찾기
                # 'time' 키를 기준으로 리스트를 정렬
                sorted_reservations = sorted(reservation_list, key=lambda x: x['time'])
                earliest_time_obj = sorted_reservations[0]['time']
                display_time = earliest_time_obj.strftime('%H:%M')
                
                # (수정) 인원 수와 가장 빠른 시간을 표시
                st.error(f"🚑 긴급 예약 ({count}명)\n\n(가장 빠른 시간: {display_time})")
            else:
                # 예약이 없으면 혼잡도 표시
